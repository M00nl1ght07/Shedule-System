...
def script_2(src: str, dst: str):
    shutil.copy2(src=src, dst=dst)
    return dst


def script_3(file_name: str, key_word: str, output_table: str):
    ...
    for i in range(1, ws1.max_row+1):
        ...
        for c in range(1, ws1.max_column+1):
            ...
            if key_word in value.split():
                #время пары
                time = del_spaces(ws1._get_cell(row=cell.row, column=2).value)
                #название группы
                print()
                print(ws1._get_cell(row=cell.row, column=2).value)
                print(time)
                print(offset[time])
                print(cell.row-1-offset[time])
                print(cell.column)
                print(del_spaces(ws1._get_cell(row=cell.row-1-offset[time], column=cell.column).value))
                print()
...
def script_4(document_name: str, key_word: str, excel_table_name: str):
                ...
                if key_word in cell.text.split():
                    ...
                    for element in changes:
                        ...
                        if key_word in element.split():
                            print('!!!!! key_word in element.split()')
                            #убирает пару из расписания
                            if changes.index(element) == 3:
                                ...
                                print()
                                print(doc_days[counter])
                                print(int(changes[0])+1)
                                print(ws[f'{doc_days[counter]}{int(changes[0])+1}'])
                                print(ws[f'{doc_days[counter]}{int(changes[0])+1}'].value)
                            ...
                            #добавляет пару в расписание
                                ...
                                print('!!!!!!!!!!! elif changes.index(element) == 5:')
                                ...
                                print('NONONONONONONONNONONONOON')

                    #закрываем соединение с таблицей                       
                    wb.close()
    return excel_table_name

def script_5(excel_table_name: str):
    #открытие таблицы вывода
    wb = load_workbook(excel_table_name)
    ws = wb.active
...
offset = {'08.45-10.15': 0, '10.30-12.00': 1, '12.40-14.10': 2, '14.20-15.50': 3, '16.00-17.30': 4, '17.40-19.10': 5, 
          '10.15-08.45': 0, '12.00-10.30': 1, '14.10-12.40': 2, '15.50-14.20': 3, '17.30-16.00': 4, '19.10-17.40': 5}
